...
void allocate_memory(list_t * freelist, list_t * alloclist, int pid, int blocksize, int policy) {
  bool is_in_list = list_is_in_by_size(freelist, blocksize);
    if (is_in_list != 1)
    {
        printf("An error occured");
        return;
    }
    int bindex = list_get_index_of_by_Size(freelist, blocksize);
    block_t *blk = list_remove_at_index(freelist, bindex);
    int block_end = blk->end;
    blk->pid = pid;
...
void deallocate_memory(list_t * alloclist, list_t * freelist, int pid, int policy) { 
  bool is_in_list = list_is_in_by_pid(alloclist, pid);
    ...
    int bindex = list_get_index_of_by_Pid(alloclist, pid);
    block_t *blk = list_remove_at_index(alloclist, bindex);
    blk->pid = 0;
    if (policy == 1)
    {
